			...
			<div className="flex justify-between items-start gap-20 flex-wrap max-lg:flex-col">
				...
				<div className="flex flex-1 justify-between lg:gap-10 gap-20 flex-wrap">
					{footerLinks.map((section) => (
						<div key={section}>
							<h4 className="text-whhite text-2xl leading-normal font-medium mb-6">{section.title} </h4>
							<ul>
								{section.links.map((link) => (
									<li key={link.name} className="mt-3 text-white-400 font-montserrat leading-normal text-base hover:text-slate-gray cursor-pointer">	<a href="">{link.name}</a></li>
								))}
							</ul>
						</div>
					))}
